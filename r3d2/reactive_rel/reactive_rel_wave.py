...
class ReactiveRelWaveSection(WaveSection):
    ...
    def deflagration_root(p_0_star, q_precursor, unknown_eos, wavenumber, label):
        ...
        q_unknown = ReactiveRelState(rho, v, vt, eps, unknown_eos, label)
    ...
    def post_discontinuity_state(p_star, q_start, lr_sign, label, j2, rho, eps, dp,
        ...
        q_end = ReactiveRelState(rho, v, vt, eps, eos_end, label=label)
...
class Contact(ReactiveRelWaveSection):
...
class Rarefaction(ReactiveRelWaveSection):
    ...
    def __init__(self, q_start, p_end, wavenumber):
        ...
        if numpy.allclose(q_start.p, p_end):
            self.trivial = True
            self.q_end = ReactiveRelState(q_start.rho, q_start.v, q_start.vt, q_start.eps,
            ...
            self.q_end = ReactiveRelState(w_all[-1, 0], w_all[-1, 1],
    ...
    def rarefaction_dwdp(w, p, q_known, wavenumber):
        ...
        local_state = ReactiveRelState(rho, v, vt, eps, q_known.eos)
        cs = local_state.cs
        h = local_state.h
    ...
    def plotting_data(self):
        ...
        if self.trivial:
            ...
            for i in range(len(p)):
                state = ReactiveRelState(w_all[i,0], w_all[i,1],
...
class Shock(ReactiveRelWaveSection):
    ...
    def __init__(self, q_start, p_end, wavenumber):
        ...
        if numpy.allclose(q_start.p, p_end):
            ...
            j2, rho, eps, dp = self.mass_flux_squared(q_start, p_end,
                                                 q_start.eos)
            v_shock, self.q_end = self.post_discontinuity_state(p_end, q_start,
                                    lr_sign, label, j2,
                                    rho, eps, dp)
...
class Deflagration(ReactiveRelWaveSection):
...
class Detonation(ReactiveRelWaveSection):
        ...
        if wavenumber == 1:
            ...
            if q_known.p < unknown_value:
                ...
                if t_known < t_i: # Need a precursor shock
                    p_min = unknown_value
                    p_max = q_known.p
                    t_min = ReactiveRelWave.precursor_root(p_min, q_known, wavenumber)
                    t_max = ReactiveRelWave.precursor_root(p_max, q_known, wavenumber)
                    assert(t_min < 0)
                    ...
                    p_0_star = brentq(ReactiveRelWave.precursor_root, p_min, p_max,
