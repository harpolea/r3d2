function wave_speeds = SR1d_GetWaveSpeeds( l, s_l, s_r, r, eos_options )

wave_speeds = zeros(5, 1);

[cs_l  , h_l  , p_l  , ~, ~, ~, ~, ~] = EOS(l(1)  , l(3)  , 1, eos_options);
[cs_s_l, h_s_l, p_s_l, ~, ~, ~, ~, ~] = EOS(s_l(1), s_l(3), 1, eos_options);
[cs_s_r, h_s_r, p_s_r, ~, ~, ~, ~, ~] = EOS(s_r(1), s_r(3), 2, eos_options);
[cs_r  , h_r  , p_r  , ~, ~, ~, ~, ~] = EOS(r(1)  , r(3)  , 2, eos_options);
