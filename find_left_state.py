...
def find_left(q_r, M=1., gamma=5./3., K=1.):
    """
    Finds the left wave for a given Mach number and right state.

    Returns the left hand state.
    """

    # calculate sound speed
    # for gamma law, h * c_s**2 = 2 gamma * p / rho
    rho_r = q_r.rho
    p_r = K * rho_r**gamma
    h = 1. + p_r * gamma / (rho_r * (gamma - 1.))
    c_s = np.sqrt(2. * gamma * p_r / (rho_r * h))
